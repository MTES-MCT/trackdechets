...
L'ensemble des champs du BSD numérique est décrit dans la [référence de l'API](../reference/api-reference/bsdasri/objects#bsdasri). Au cours de son cycle de vie, un BSD numérique peut passer par différents états décrits [ici](../reference/api-reference/bsdasri/enums#bsdasristatus).
