...
@registerUpdater(
  ...
  "As BODY_BENNE is deprecated, we edit bsdas with this packaging and change them to BIG_BAG (equivalent)",
  true
)
  ...
  async run() {
    try {
      ...
      for (const rowToUpdate of rowsToUpdate) {
        ...
        const bigBagPackaging = newPackagings.find(p => p.type === "BIG_BAG");
