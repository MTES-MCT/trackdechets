...
const CREATE_BSVHU_TRANSPORTER = gql`
...
describe("Mutation.createBsvhuTransporter", () => {
  ...
  it("should create a BSVHU transporter", async () => {
    const { user } = await userWithCompanyFactory("MEMBER");
    const { mutate } = makeClient(user);
    const transporter = await companyFactory({
      companyTypes: ["TRANSPORTER"],
    ...
    expect(data.createBsvhuTransporter!.id).toBeDefined();

    const bsvhuTransporter = await prisma.bsvhuTransporter.findUniqueOrThrow({
      where: { id: data.createBsvhuTransporter!.id }
    });

    expect(bsvhuTransporter).toMatchObject({
  ...
  it("should auto-complete name and address from SIRENE database", async () => {
    ...
    expect(bsvhuTransporter.transporterCompanyName).toEqual(searchResult.name);
    expect(bsvhuTransporter.transporterCompanyAddress).toEqual(
