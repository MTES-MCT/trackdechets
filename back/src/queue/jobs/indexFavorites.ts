 ...
 */
const buildBsdsQueryBody = (orgId: string, field: string) => ({
  index: index.alias,
  size: MAX_FAVORITES,
  body: {
    query: {
...
async function getRecentTransporters(orgId: string) {
  const transporterCompanySiret = buildBsdsQueryBody(
    orgId,
    "transporterCompanySiret"
  );
  const transporterCompanyVatNumber = buildBsdsQueryBody(
    orgId,
    "transporterCompanyVatNumber"
  );

  const [{ body }, { body: vatBody }] = await Promise.all([
    client.search(transporterCompanySiret),
    client.search(transporterCompanyVatNumber)
  ]);
  ...
  if (isMatchingType && !isAlreadyListed) {
    try {
      ...
      if (err instanceof AnonymousCompanyError) {
        // And it's an anonymous that is not found in AnonymousCompany table
        favorites.push(
