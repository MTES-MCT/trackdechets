...
describe("Retrieval of bsds in ES based on waste register type", () => {
  ...
  async function searchBsds(registerType: WasteRegisterType, sirets: string[]) {
    const { body } = await client.search({
      index: index.alias,
      body: {
        sort: { createdAt: "ASC" },
        query: buildQuery(registerType, sirets, {})
      }
    });
