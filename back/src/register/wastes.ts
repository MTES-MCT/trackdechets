...
/**
 * Perform the actual data fetching and return a waste connection
 * @param registerType type de registre : entrant, sortant, transport, etc
...
async function getWasteConnection<WasteType extends Waste>(
  ...
  const searchHits = await searchBsds(registerType, args.sirets, args.where, {
    size,
    sort,
  ...
  const wastes = toWastes<WasteType>(registerType, args.sirets, bsds);
