...
import { OrderType, WasteRegisterType } from "../generated/graphql/types";

function buildSort(
  registerType: WasteRegisterType,
  order: OrderType
): { [key in keyof BsdElastic]?: OrderType }[] {
  const sortKey: { [key in WasteRegisterType]: keyof BsdElastic } = {
...
export async function getElasticPaginationArgs({
  ...
  registerType,
  ...args
}: PaginationArgs & { registerType: WasteRegisterType }) {
  ...
  const sort = buildSort(registerType, order);
