...
const segmentSchema = Yup.object<any>().shape({
  ...
  transporterValidityLimit: Yup.date()
    .typeError(
      "La date de validité du récépissé transporteur  n'est pas formatée correctement"
    )
    .nullable()
    .notRequired(),
...
const takeOverInfoSchema = Yup.object<any>().shape({
  takenOverAt: Yup.date()
    .typeError("La date de prise en charge n'est pas formatée correctement")
    .required(),
