...
export const prepareRedis = async ({ emitterCompany, recipientCompany }) => {
  await storeRedisCompanyInfo({company: emitterCompany, companyTypes:  ["PRODUCER"]})
  await storeRedisCompanyInfo({company: recipientCompany, companyTypes:  ["WASTE_PROCESSOR"]})
