...
describe("query favorites", () => {
  ...
  it("should return favorites of foreign companies when allowForeignCompanies is true", async () => {
    ...
    searchCompanySpy.mockResolvedValueOnce({
      ...convertUrls(company2)
    });
    await indexFavorites(
      await favoritesConstrutor({ orgId: company2.orgId, type: "TRANSPORTER" }),
      { orgId: company2.orgId, type: "TRANSPORTER" }
    );
    const { query } = makeClient({ ...user, auth: AuthType.Session });
    const { data } = await query<Pick<Query, "favorites">>(FAVORITES, {
      variables: {
    ...
    const { query: query2 } = makeClient({ ...user2, auth: AuthType.Session });
    const { data: data2 } = await query2<Pick<Query, "favorites">>(FAVORITES, {
      variables: {
        orgId: company2.orgId,
        type: "TRANSPORTER",
        allowForeignCompanies: true
  ...
  it("should NOT return favorites of foreign companies when allowForeignCompanies is false", async () => {
    const { user, company } = await userWithCompanyFactory("MEMBER", {
