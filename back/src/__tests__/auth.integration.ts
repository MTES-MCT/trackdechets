...
describe("Second factor", () => {
  ...
  it("should deny user when otp is wrong", async () => {
    ...
    expect(secondFactor.header.location).toBe(
      `http://${UI_HOST}/second-factor?errorCode=INVALID_TOTP&lockout=5`
    );
  ...
  it("should increase lockout when otp is wrong", async () => {
    ...
    expect(secondFactor.header.location).toBe(
      `http://${UI_HOST}/second-factor?errorCode=INVALID_TOTP&lockout=10`
    );
  ...
  it("should ignore totp during lockout", async () => {
    ...
    expect(secondFactor.header.location).toBe(
      `http://${UI_HOST}/second-factor?errorCode=TOTP_LOCKOUT&lockout=5` // matches totpLockedUntil 5 seconds
    );
  });
