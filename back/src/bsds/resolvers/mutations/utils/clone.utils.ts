...
export const cloneBsdd = async id => {
  ...
  const newBsddCreateInput: Prisma.FormCreateInput = {
    ...
    wasteDetailsPackagingInfos: JSON.stringify(bsdd.wasteDetailsPackagingInfos),
    wasteDetailsParcelNumbers: JSON.stringify(bsdd.wasteDetailsParcelNumbers),
  ...
  const newBsdd = await prisma.form.create({ data: newBsddCreateInput });

  console.log(newBsdd);

  await enqueueUpdatedBsdToIndex(newBsdd.id);
};
