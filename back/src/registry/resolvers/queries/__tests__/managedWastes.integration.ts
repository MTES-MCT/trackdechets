...
describe("Managed wastes registry", () => {
  ...
  it("should return an error when querying a SIRET the user is not member of", async () => {
    ...
    expect(errors[0]).toEqual(
      expect.objectContaining({
        message: `Vous n'êtes pas autorisé à accéder au registre de l'établissement portant le SIRET ${broker.company.siret}`
      })
    );
  });
  ...
  it("should not accept service account connection from IP address not in the white list", async () => {
    ...
    expect(res.body).toEqual({
      ...
      errors: [
        expect.objectContaining({
          message: `Vous n'êtes pas autorisé à accéder au registre de l'établissement portant le SIRET ${trader.company.siret}`
        })
      ]
    });
  });
