...
export default function ReceivedInfo({
  ...
  const initialIsTempStorage =
    form.status === FormStatus.Sent && !!form.recipient?.isTempStorage;
  ...
  return (
    <Formik<ReceivedInfoValues>
      initialValues={{
        ...
        isTempStorage: initialIsTempStorage,
      }}
      onSubmit={onSubmit}
      ...
      {({ values, isSubmitting, handleReset, setFieldValue }) => {
        function handleTempStorageToggle(checked) {
          if (checked) {
            ...
            setFieldValue("isTempStorage", true);
          } else {
            // the switch is toggled off, set isTempStorage to false
            setFieldValue("isTempStorage", false);
          }
        }
        ...
        return (
          <Form>
            {form.status === FormStatus.Sent && (
              <TdSwitch
                checked={values.isTempStorage}
                onChange={handleTempStorageToggle}
                label="Il s'agit d'une réception sur une installation d'entreposage provisoire ou de reconditionnement"
              />
            )}
