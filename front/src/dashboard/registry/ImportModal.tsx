...
const steps = [
  ...
  { title: "Confirmation", component: Step1 }
];

const REGISTRY_SIGNED_URL = gql`
  query RegistrySignedUrl($fileName: String!) {
    registrySignedUrl(fileName: $fileName) {
      fileKey
      signedUrl
    }
  }
  ...
  return (
    <div>
      <Alert
        ...
        description="Vous trouverez de l'aide sur le format de fichier et des exemples dans la documentation"
        severity="info"
      />
...
function Step2({ getValues, goToNextStep }: StepProps) {
  ...
  const { error: signedUrlError } = useQuery<
    Pick<Query, "registrySignedUrl">,
    Partial<QueryRegistrySignedUrlArgs>
  >(REGISTRY_SIGNED_URL, {
    variables: { fileName: file.name },
    onCompleted: async ({ registrySignedUrl }) => {
      const uploadResponse = await fetch(registrySignedUrl.signedUrl, {
        method: "PUT",
        headers: {
      ...
      if (uploadResponse.ok) {
        await importFile({
          variables: { importType: type, s3FileKey: registrySignedUrl.fileKey }
        });
      } else {
