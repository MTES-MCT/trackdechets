...
export function RegistryIncomingWasteForm({ onClose }: Props) {
  ...
  useQuery<Pick<Query, "registryLookup">>(GET_INCOMING_WASTE_REGISTRY_LOOKUP, {
    variables: {
    ...
    onCompleted: data => {
      if (data?.registryLookup?.incomingWaste) {
        ...
        const { publicId: _, ...propsWithoutPublicId } = definedIncomingWasteProps;

        // Set the form values with the transformed data
        const resetValues = {
          ...DEFAULT_VALUES,
          ...(isDuplicate ? propsWithoutPublicId : definedIncomingWasteProps),
