...
import FormBsvhuSignTransport from "./FormBsvhuSignTransport";

const SignTransportModal = ({ bsvhuId, siret, isOpen, onClose }) => {
  ...
  return bsvhu ? (
    <TdModal onClose={onClose} ariaLabel={title} isOpen={isOpen}>
      ...
      {bsvhu.metadata?.errors?.some(
        ...
        <FormBsvhuSignTransport bsvhu={bsvhu} onClose={onClose} />
      )}
    </TdModal>
  ) : (
    <Loader />
