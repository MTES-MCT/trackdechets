...
import FormBsdaSignEmission from "./FormBsdaSignEmission";

const SignEmissionModal = ({ bsdaId, siret, isOpen, onClose }) => {
  ...
  return bsda ? (
    <TdModal onClose={onClose} ariaLabel={title} isOpen={isOpen}>
      ...
      {bsda.metadata?.errors?.some(
        ...
        <FormBsdaSignEmission bsda={bsda} onClose={onClose} />
      )}
    </TdModal>
  ) : (
    <Loader />
