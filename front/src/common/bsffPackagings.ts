import { BsffPackaging } from "@td/codegen-ui";
...
export function mergeBsffPackagings<P extends BsffPackaging>(
  packagings: P[]
): P[] {
  const packagingMap: { [key: string]: P } = {};

  for (const packaging of packagings) {
    const { type, volume, other, weight, numero } = packaging;
    ...
    if (existingValue) {
      packagingMap[mapKey].weight += weight;
    } else {
      packagingMap[mapKey] = {
        type,
        volume,
