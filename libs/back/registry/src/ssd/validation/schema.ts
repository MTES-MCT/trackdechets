...
const wasteCodeSchema = z.string().refine(val => {
  // We don't have a proper enum type (z.enum) to use here as the codes are defined in a tree
  return BSDD_WASTE_CODES.includes(val);
}, "Le code déchet n'est pas valide. Il doit faire partie de la liste officielle des codes déchets. Ex: 17 02 01, 10 01 18*. Attention à bien respecter les espaces.");
    ...
    val =>
      val
        ? String(val)
            ...
            .map(val => val.trim())
        : [],
    z.array(wasteCodeSchema).optional()
  ),
